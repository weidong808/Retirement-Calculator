...
export function AppLogo({ size = 48, showWordmark = false, variant = "light" }: AppLogoProps) {
  const gradId = useId();
  const sunId = useId();
  const textColor = variant === "light" ? "#ffffff" : "#1a3a5c";
  const subColor = variant === "light" ? "rgba(255,255,255,0.88)" : "#64748b";

  return (
    <div
      className={`app-logo${showWordmark ? " app-logo--stacked" : ""}`}
    ...
    >
      ...
      >
        ...
        {/* Horizon & landscape — retirement ahead */}
        <path
      ...
      {showWordmark && (
        <div className="app-logo-text">
          <span className="app-logo-title" style={{ color: textColor }}>
            Retire<span className="app-logo-accent">Check</span>
          </span>
